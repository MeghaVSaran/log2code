# Log Parser — Walkthrough

## Changes Made

### [log_parser.py](file:///c:/projects/log2code/src/ingestion/log_parser.py)

Implemented 4 functions with regex-based pattern matching:

| Function | What it does |
|----------|-------------|
| [extract_error_type()](file:///c:/projects/log2code/src/ingestion/log_parser.py#141-158) | Priority-ordered matching: include → template → linker → compiler → segfault → unknown |
| [extract_identifiers()](file:///c:/projects/log2code/src/ingestion/log_parser.py#160-207) | Extracts symbols from 5 patterns; capture group uses `[A-Za-z_~][A-Za-z0-9_:<>~*]*` to stop before `(` |
| [extract_file_hints()](file:///c:/projects/log2code/src/ingestion/log_parser.py#209-239) | Matches C/C++ filenames (`.cpp`, `.h`, `.hpp`, etc.) via general + include-error patterns |
| [parse_log()](file:///c:/projects/log2code/src/ingestion/log_parser.py#112-139) | Orchestrates above 3 + picks most informative line + extracts stack frames |
...
### [test_log_parser.py](file:///c:/projects/log2code/tests/test_log_parser.py) [NEW]

9 test classes, 30 individual test methods.
...
```
tests/test_log_parser.py::TestLinkerError           — 5 passed
tests/test_log_parser.py::TestCompilerError          — 5 passed
tests/test_log_parser.py::TestIncludeError           — 3 passed
tests/test_log_parser.py::TestTemplateError          — 2 passed
tests/test_log_parser.py::TestSegfault               — 4 passed
tests/test_log_parser.py::TestMultilineLog           — 3 passed
tests/test_log_parser.py::TestUnknownError           — 3 passed
tests/test_log_parser.py::TestMultipleErrorLines     — 2 passed
tests/test_log_parser.py::TestNamespacedIdentifiers  — 3 passed
```

### Manual LLVM log verification ✅

```
Input:  undefined reference to `llvm::SelectionDAG::getNode(unsigned int, llvm::SDLoc const&, llvm::EVT)'
Output: identifiers=['llvm::SelectionDAG::getNode']  ← full namespace, no parameter list
```
